...
mod tests {
    ...
    #[test]
    fn test_trie_siblings_with_branching() {
        // FAILS AS EXPECTED - BUG FOUND
        //
        // Goal: Trigger a bug in Trie sibling creation when multiple Trie siblings branch from a Name node.
        //
        // Scenario: Pointers "/foo", "/foa", "/fob" should create:
        //   - Name("fo") as the longest common prefix
        //   - Trie("o") for "/foo"
        //   - Trie("a") for "/foa"
        //   - Trie("b") for "/fob"
        //
        // Expected: The tree should have 5 nodes (Root + Name + 3 Trie siblings).
        // Name("fo") should have num_trie_children = 3 and child_index pointing to first Trie.
        // All three Tries should be siblings at the same level with parent = Name("fo").
        //
        // Actual bug: The algorithm creates Name("foa") with a match and drops the other pointers.
        // It only creates 2 nodes total (Root + Name("foa")).
        //
        // Root cause: The Name node creation logic (around line 360) only creates new Name siblings
        // when common_prefix_len == 0. It doesn't handle the case where there's a partial common
        // prefix that should result in a shorter Name node with Trie children.
        //
        // Suggested fix: After creating Name roots, check if any Name node has siblings with
        // common prefixes > 0. If so, find the longest common prefix among all siblings, shorten
        // the Name node to that prefix, and create Trie children for the diverging suffixes.
        // Alternatively, compute the longest common prefix for each group of adjacent pointers
        // BEFORE creating the Name node.

        let pointers = ["/foo", "/foa", "/fob"]
            .into_iter()
            .map(|s| Pointer::from_static(s))
            .collect::<Vec<_>>();
        let g = Group::from_pointers(pointers);

        let expect_nodes = vec![
            Node::default().with_child_index(1).with_name_children(1),
            Node::new_name("fo", None)
                .with_child_index(2)
                .with_trie_children(3),
            Node::new_trie("a", Some(Pointer::from_static("/foa"))),
            Node::new_trie("b", Some(Pointer::from_static("/fob"))),
            Node::new_trie("o", Some(Pointer::from_static("/foo"))),
        ];
        let expect_parents = vec![0, 1, 1, 1];
    ...
    #[test]
    fn test_two_pointers_with_partial_common_prefix() {
        // FAILS AS EXPECTED - BUG FOUND
        //
        // Goal: Trigger a bug with just two pointers that have a partial common prefix.
        //
        // Scenario: Pointers "/ab", "/ac" should create:
        //   - Name("a") as the common prefix
        //   - Trie("b") for "/ab"
        //   - Trie("c") for "/ac"
        //
        // Expected: The tree should have 4 nodes (Root + Name + 2 Trie siblings).
        // Name("a") should have num_trie_children = 2.
        //
        // Actual bug: Same as previous test - creates Name("ab") with match and drops "/ac".
        //
        // Suggested fix: Same as previous test - the Name node creation needs to compute
        // the longest common prefix among all pointers in the range [start_index, end_index)
        // at the current level, then create Name nodes with that prefix length and Trie
        // children for the diverging suffixes.
        ...
        let expect_nodes = vec![
            Node::default().with_child_index(1).with_name_children(1),
            Node::new_name("a", None)
                .with_child_index(2)
                .with_trie_children(2),
            Node::new_trie("b", Some(Pointer::from_static("/ab"))),
            Node::new_trie("c", Some(Pointer::from_static("/ac"))),
        ];
        let expect_parents = vec![0, 1, 1];
