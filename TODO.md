...
Still need to do something about the content fetch path, which remains much slower than the same
path for `FixedAnalyzer`.
...
5. ✅ ~~Experiment with replacing the `match (token, expect)`  expression with a lookup table with
   an OR'd lookup key.~~
6. ❌ ~~If NOT doing lookup table, remove match guards `a if x == y =>` from the match statements
   and replace with OR patterns, `a | b | c`. This is supposed to be more efficient.~~ Went with
   lookup table.
